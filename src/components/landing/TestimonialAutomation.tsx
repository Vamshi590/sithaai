...
import { SparkleIcon, InstagramIcon, SendIcon, ShieldIcon } from './PremiumIcons'
...
export default function TestimonialAutomation() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      window.requestAnimationFrame(() => {
        ...
        const progress = Math.min(Math.max(-rect.top / (height - viewHeight), 0), 1);
  ...
  const activeIndex = scrollProgress < 0.25 ? 0 : scrollProgress < 0.5 ? 1 : scrollProgress < 0.75 ? 2 : 3

  const scenes = [
    { title: 'Zero-Friction Capture.', sub: 'CLINIC WORKFLOW', desc: 'Securely document patient successes. A staff member snaps a photo and sends it to your unified WhatsApp Assistant.' },
    { title: 'Empathetic Narratives.', sub: 'MEDICAL AI', desc: 'Sitha extracts the clinical context and localizes the patient\'s success story into 12+ regional languages to build deep community trust.' },
    { title: 'Hospital-Grade Branding.', sub: 'BRAND ENGINE', desc: 'The AI instantly wraps the raw photo in your practice\'s premium visual identity, creating high-authority proof of care.' },
    { title: 'Drive Patient Acquisition.', sub: 'SOCIAL FOOTFALL', desc: 'Review on WhatsApp. Tap "Approve" to instantly broadcast the testimonial to Instagram and attract new patients on autopilot.' }
  ]
    ...
    >
      <div className="sticky top-0 h-screen w-full flex flex-col md:flex-row bg-white z-[5] overflow-hidden">
        <div className="absolute inset-0 -z-10 bg-white">
           <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[2000px] h-[1000px] rounded-full blur-[200px] transition-all duration-1000 ${activeIndex === 0 ? 'bg-emerald-500/5' : activeIndex === 1 ? 'bg-indigo-500/5' : activeIndex === 2 ? 'bg-brand-cyan/10' : 'bg-brand-cyan/5'}`} />
        </div>
        ...
        <div className="w-full md:w-[45%] h-full flex flex-col justify-center px-12 md:px-24 relative z-50 bg-white/40 backdrop-blur-3xl border-r border-slate-100/50">
                ...
                return (
                  ...
                  >
                     <div className="flex items-center gap-4 mb-8">
                        ...
                        <span className="text-brand-cyan text-[10px] font-black uppercase tracking-[0.6em]">{scene.sub}</span>
                     </div>
           ...
           <div className="absolute bottom-20 left-12 md:left-24 flex gap-4">
              {scenes.map((_, i) => (
                <div key={i} className={`h-1 transition-all duration-500 ${activeIndex === i ? 'w-12 bg-brand-cyan shadow-[0_0_15px_rgba(0,212,255,0.4)]' : 'w-4 bg-slate-100'}`} />
              ))}
           </div>
        </div>
        ...
        <div className="flex-1 h-full relative flex items-center justify-center p-12 lg:p-24 overflow-hidden perspective-1000">
           ...
           <div className="absolute inset-0 flex items-center justify-center transition-all duration-500"
                style={{ opacity: p1, transform: `scale(${0.9 + p1 * 0.1}) rotateY(${(1-p1)*20}deg)` }}>
              <div className="w-[450px] h-[550px] bg-white rounded-[60px] shadow-2xl border border-slate-100 p-4 relative pointer-events-none group">
                 ...
                 <div className="absolute bottom-8 left-8 right-8 bg-white/90 backdrop-blur-xl p-5 rounded-[24px] shadow-2xl border border-white flex items-center gap-4">
                    <div className="w-12 h-12 bg-emerald-100 rounded-2xl flex items-center justify-center text-xl shadow-sm">📸</div>
                    <div>
                        <p className="text-slate-950 font-bold text-sm tracking-tight mb-0.5">Snap & Send</p>
                        <p className="text-slate-500 text-xs font-medium">Directly via WhatsApp</p>
                    </div>
                 </div>
              </div>
           </div>
           ...
           {/* ASSET 2 */}
           <div className="absolute inset-0 flex items-center justify-center transition-all duration-500"
                style={{ opacity: p2, transform: `translateY(${(1-p2)*100}px) scale(${0.8 + p2 * 0.2})` }}>
              <div className="flex gap-8 w-full max-w-4xl px-20 pointer-events-none">
                 ...
                 <div className="flex-1 self-center relative">
                    <div className="bg-slate-950 p-12 rounded-[60px] shadow-2xl rotate-[5deg] relative z-10">
                       <p className="text-2xl text-white font-black italic leading-tight">"I was nervous about the procedure, but the care and personal attention I received here truly changed my life."</p>
                    </div>
                 </div>
              </div>
           </div>
           ...
           <div className="absolute inset-0 flex items-center justify-center transition-all duration-500"
                style={{ opacity: p3, transform: `scale(${0.85 + p3 * 0.15}) rotate(${-(1-p3)*10}deg)` }}>
              <div className="w-[450px] aspect-[4/5] bg-white rounded-[60px] shadow-4xl border border-white/20 relative overflow-hidden group">
                 ...
                 <div className="absolute bottom-0 left-0 w-full p-10 bg-gradient-to-t from-black/60 via-black/20 to-transparent">
                    <p className="text-xl text-white font-bold italic leading-tight drop-shadow-md">
                       "I was nervous about the procedure, but the care and personal attention I received here truly changed my life."
                    </p>
                 </div>
           ...
           <div className="absolute inset-0 flex items-center justify-center transition-all duration-700"
                style={{ opacity: p4, transform: `translateY(${(1-p4)*200}px) scale(${0.8 + p4 * 0.2})` }}>
                 ...
                 <div className="w-full max-w-[600px] aspect-[4/5] bg-white rounded-[60px] shadow-4xl border border-white/20 relative overflow-hidden transition-all">
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-12 bg-gradient-to-t from-black/70 via-black/30 to-transparent">
                       <p className="text-2xl text-white font-bold italic leading-tight mb-4 shadow-sm">
                          "I was nervous about the procedure, but the care and personal attention I received here changed my life."
                       </p>
                    </div>
                 </div>
                 ...
                 <div className="absolute -right-20 bottom-20 w-96 transform rotate-[-3deg] animate-[float_6s_ease-in-out_infinite]">
                    <div className="bg-white/95 backdrop-blur-3xl p-10 rounded-[60px] shadow-4xl border border-slate-100">
                       <div className="flex items-center gap-4 mb-8">
                          ...
                          <div>
                             <span className="text-[10px] font-black text-slate-400 tracking-[0.3em] block mb-1 uppercase">Instant Growth</span>
                             <span className="text-sm font-bold text-slate-900">Sitha AI Assistant</span>
                          </div>
                       </div>
                       <p className="text-lg font-bold text-slate-950 mb-8 leading-tight">Your story is ready for 1,200+ reach. Reply 1 to post.</p>
                       <button className="w-full py-6 bg-slate-950 text-white rounded-[35px] text-[12px] font-black uppercase tracking-[0.4em] shadow-3xl hover:bg-slate-800 transition-all hover:scale-[1.05]">Approve & Post</button>
                    </div>
                 </div>
